...
def _img_rescaling_aug(image, label=None, scale=None, aug_img=None):
    ...
    new_image_aug = Image.fromarray(aug_img.astype(np.uint8)).resize(new_scale, resample=Image.BILINEAR)

    # new_image = np.asarray(new_image).astype(np.float32)
    new_image = np.asarray(new_image)
    new_image_aug = np.asarray(new_image_aug)

    if label is None:
        return new_image,new_image_aug

    new_label = Image.fromarray(label).resize(new_scale, resample=Image.NEAREST)
    new_label = np.asarray(new_label)

    return new_image, new_label,new_image_aug


def random_scaling_fgaug(image, label=None, scale_range=None,aug_img=None):
    min_ratio, max_ratio = scale_range
    assert min_ratio <= max_ratio

    ratio = random.uniform(min_ratio, max_ratio)

    return _img_rescaling_aug(image, label, scale=ratio, aug_img=aug_img)
...
def random_fliplr_fgaug(image, label=None,aug_img=None):
    p = random.random()
...
def random_crop(image, label=None, crop_size=None, mean_rgb=[0, 0, 0], ignore_index=255):
    ...
    pad_mask[H_pad:(H_pad + h), W_pad:(W_pad + w)] = 1
    ...
    mask = pad_mask[H_start:H_end, W_start:W_end]
    ...
    return image, label, img_box, mask

def random_crop_fgaug(image, label=None, crop_size=None, mean_rgb=[0, 0, 0], ignore_index=255,aug_img=None):
    h, w, _ = image.shape
    ...
    pad_image_aug = np.zeros((H, W, 3), dtype=np.float32)
    pad_image_aug = pad_image_aug.astype(np.uint8)
    pad_image_aug[:, :, 0] = mean_rgb[0]
    pad_image_aug[:, :, 1] = mean_rgb[1]
    pad_image_aug[:, :, 2] = mean_rgb[2]
    pad_image_aug[H_pad:(H_pad + h), W_pad:(W_pad + w), :] = aug_img
    ...
    image = pad_image[H_start:H_end, W_start:W_end, :]
    aug_img = pad_image_aug[H_start:H_end, W_start:W_end, :]
    ...
    if label is None:
        return image, img_box, mask, aug_img
    ...
    return image, label, img_box, mask, aug_img
...
def add_gaussian_noise_fgaug(image_in, noise_sigma,aug_img=None):
    """
    给图片添加高斯噪声
    image_in:输入图片
    noise_sigma：
    """
    temp_image = np.float64(np.copy(image_in))
    temp_image_aug = np.float64(np.copy(aug_img))
    ...
    noisy_image = np.zeros(temp_image.shape, np.float64)
    noise_aug_img = np.zeros(temp_image_aug.shape, np.float64)

    if len(temp_image.shape) == 2:
        noisy_image = temp_image + noise
        noise_aug_img = temp_image_aug + noise

    else:
        noisy_image[:, :, 0] = temp_image[:, :, 0] + noise
        noisy_image[:, :, 1] = temp_image[:, :, 1] + noise
        noisy_image[:, :, 2] = temp_image[:, :, 2] + noise
        noise_aug_img[:, :, 0] = temp_image_aug[:, :, 0] + noise
        noise_aug_img[:, :, 1] = temp_image_aug[:, :, 1] + noise
        noise_aug_img[:, :, 2] = temp_image_aug[:, :, 2] + noise

    return noisy_image,noise_aug_img