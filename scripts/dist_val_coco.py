...
from utils.camutils import (multi_scale_cam, cam_to_label,refine_cams_with_bkg_v2,refine_cams_with_bkg_val)
...
if __name__ == "__main__":
    ...
    if args.local_rank == 0:
        setup_logger(filename=os.path.join('/amax/sungy/project/e2edl/work_dir_attn_coco', 'val_coco_'+ timestamp + '.log'))
        logging.info('\nargs: %s' % args)
        logging.info('\nconfigs: %s' % cfg)
    ...
    with torch.no_grad():
        ...
        for _, data in tqdm(enumerate(val_loader),
            ...
            # inputs = np.asarray(imageio.imread(os.path.join(cfg.dataset.root_dir,'JPEGImages','val',name[0] + '.jpg')))
            inputs = robust_read_image(os.path.join(cfg.dataset.root_dir,'JPEGImages','val',name[0] + '.jpg'))
            ...
            ori_img = np.array(Image.open(os.path.join(cfg.dataset.root_dir, 'JPEGImages','val', name[0] + '.jpg')).convert("RGB"))
