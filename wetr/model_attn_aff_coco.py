...
class WeTr(nn.Module):
    def __init__(self, backbone, num_classes=None, embedding_dim=256, stride=None, pretrained=None, pooling=None,queue_len=300,momentum=0.9, ):
        ...
        # self.attention4 = CBAM(self.in_channels[3])
        # self.attention3 = CBAM(self.in_channels[2])
        # self.attention2 = CBAM(self.in_channels[1])
        # self.attention1 = CBAM(self.in_channels[0])
    ...
    def get_dl_logit_aff_cross(self,hie_fea,pseudo_label,dl_aff,inputs_denorm):
        ...
        seeds = torch.nn.functional.one_hot(pseudo_label, num_classes=self.num_classes).permute(0,3,1,2)#生成每个类别包括 2 21 128 128
        ...
        prototypes = F.adaptive_avg_pool2d(crop_feature.reshape(-1, dim, h, w), (1, 1)).view(n, 81, dim)
